...
use crate::chunk::{Chunk, MINIMUM_CHUNK_SIZE};
...
impl Png {
  ...
  pub fn remove_chunk(&mut self,chunk_type:&str)->Result<Chunk>{
    let pulled_chunk_position = self.get_chunk_position_by_type(chunk_type)?;
    Ok(self.chunks.remove(pulled_chunk_position))
  }
  ...
  pub fn chunk_by_type(&self,chunk_type:&str)->Option<&Chunk>{
    let looked_for_position = self.get_chunk_position_by_type(chunk_type).ok()?;
    Some(self.chunks.get(looked_for_position)?)
  }
  pub fn as_bytes(&self)->Vec<u8>{
    let mut vec_bytes = self.header().to_vec();
    let mut chunks_bytes:Vec<u8> = self.chunks().iter().map(|chunk|chunk.as_bytes()).into_iter().flatten().collect();
    vec_bytes.append(&mut chunks_bytes);
    vec_bytes
  }
  fn get_chunk_position_by_type(&self,chunk_type:&str)->Result<usize>{
    let tested_type = ChunkType::from_str(chunk_type)?;
    let pulled_chunk_position = self.chunks
      .iter().position(|chunk|chunk.chunktype == tested_type).context("Cant find the required chunk")?;
    Ok(pulled_chunk_position)
  }
}
impl TryFrom<&[u8]> for Png{
  type Error =Error;
  fn try_from(value: &[u8]) -> Result<Self> {
    let (header_byte,remainder) = value.split_at(Png::STANDARD_HEADER.len());
    let header:[u8;8] = header_byte.try_into()?;
    ensure!(header == Png::STANDARD_HEADER,"Invalid header to file");
    let mut chunks = Vec::new();
    let mut offset = 0;
    while offset<remainder.len(){
      let length_bytes = remainder.get(offset..offset+4).context("Can't get the length out of the first four bytes")?;
      let length = u32::from_be_bytes(length_bytes.try_into().context("couldn't parse the 4 first bytes as a U32 number")?);
      chunks.push(Chunk::try_from(remainder.get(offset..offset + length as usize + MINIMUM_CHUNK_SIZE  ).context("failed to convert a part of the list to a chunk")?)?);
      offset = offset + MINIMUM_CHUNK_SIZE + length as usize;
    }
